...
a more exciting piece of software.
...
If using a reference counter was all you needed, several people working at Oracle would be out
of a job.
...
- **Undetectable moves**. When a value is moved in Rust, there is no way to detect that fact from
  ...
  At this rate, though, we would reinvent C++'s copy constructors.
...
- **Type erasure**. In a typical Rust program, generics are implemented via monomorphization, and
  ...
  This makes it harder to clean up an allocation once it's done effectively.
...
It should be clear to see why these definitions are helpful to us.
...
I won't include the implementation of `cleanup` here because it's lengthy, and it's not even 
the final revision of the cleanup algorithm, but you can trust me that it works.
