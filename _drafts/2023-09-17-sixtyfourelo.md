...
positions to valuable evaluation data.
...
pub struct TTEntry {
    ...
    /// If the depth is negative, it was a special type of search.
    pub depth: i8, // 1 byte
...
for each position, but it requires 32 bits - not very efficient.

In my engine, each evaluation is a 16-bit integer.
Meanwhile, there are three possible cases for the type of evaluation:
...
/// These determine whether a bound is binding during a search.
pub enum BoundType {
...
enum EntryType {
    ...
    /// An extant entry is an upper bound on a position evaluation.
    _Upper = BoundType::Upper as u8,
    /// An extant entry that is an exact bound on a position evaluation.
...
search to compare against alpha and beta.

When inserting into the transposition table, we must also decide how to tag our evaluations.
...
When I originally wrote my quiescent search, I noticed that a quiescent search doesn't visit
...
You can safely ignore the rest of the ceremony, but please remember what we've passed in as a bound type: if
...
Currently, it's still not even beating out some simple engines - the kind that haven't even
